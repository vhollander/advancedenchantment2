...
public class Insomnia extends AEEnchantable {
    public Insomnia() {
    ...
    public void onTargetDamaged(net.minecraft.entity.LivingEntity user, net.minecraft.entity.Entity target, int level) {
        ((LivingEntity)target).addStatusEffect(new StatusEffectInstance(StatusEffects.NAUSEA, (int)(level/3) * 400, 0, false, true, true));
        ((LivingEntity)target).addStatusEffect(new StatusEffectInstance(StatusEffects.MINING_FATIGUE, (int)(level/3) * 400, level - 1, false, true, true));
        ((LivingEntity)target).addStatusEffect(new StatusEffectInstance(StatusEffects.SLOWNESS, level * 400, level - 1, false, true, true));
    }
