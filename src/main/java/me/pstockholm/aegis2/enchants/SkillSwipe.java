...
public class SkillSwipe extends AEEnchantable {
    public SkillSwipe() {
    ...
    public void onTargetDamaged(net.minecraft.entity.LivingEntity user, net.minecraft.entity.Entity target, int level) {
        if ( MathHelper.nextInt(user.getWorld().getRandom(), 0, 10 / level ) == 0) {
            if(target.isPlayer()) {
                int oldxp = ((PlayerEntity)target).totalExperience;
                ((PlayerEntity)target).addExperience(-(level * 15));
                if(user.isPlayer())
                    ((PlayerEntity)user).addExperience(oldxp - ((PlayerEntity)target).totalExperience );
            }
        }
    }
