...
public class Frozen extends AEEnchantable {
    public Frozen() {
    ...
    public void onUserDamaged(net.minecraft.entity.LivingEntity user, net.minecraft.entity.Entity target, int level) {
        if(user.isBlocking())
            ((LivingEntity)target).addStatusEffect(new StatusEffectInstance(StatusEffects.SLOWNESS, 50 * level, 0, false, true, true));
    }
