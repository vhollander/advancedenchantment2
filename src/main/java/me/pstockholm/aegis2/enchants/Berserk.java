...
public class Berserk extends AEEnchantable {
    public Berserk() {
    ...
    public void onTargetDamaged(net.minecraft.entity.LivingEntity user, net.minecraft.entity.Entity target, int level) {
        if ( MathHelper.nextInt(user.getWorld().getRandom(), 0, 10 / level ) == 0) {
            ((LivingEntity)user).addStatusEffect(new StatusEffectInstance(StatusEffects.STRENGTH, level * 1000, level, false, true, true));
