...
public class RocketEscape extends AEEnchantable {
    public RocketEscape() {
        super(Rarity.UNCOMMON, EnchantmentTarget.ARMOR, new EquipmentSlot[] {EquipmentSlot.FEET});
        bUseFeet = true;
    ...
    public void onUserDamaged(net.minecraft.entity.LivingEntity user, net.minecraft.entity.Entity target, int level) {
        if (user.getHealth() < user.getMaxHealth() * 0.4) {
            user.takeKnockback(level, user.getPos().x + 25, user.getPos().z + 25);
        }
    }
